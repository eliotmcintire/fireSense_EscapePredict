...
#    `functionName` is a function's name in your module (e.g., `fireSense_EscapePredictEvent1`).
# 2. Copy this file to the tests folder (i.e., ` # replace with empty string insteadfireSense_EscapePredict/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_EscapePredict")
  ...
  parameters <- list(
    ...
    fireSense_EscapePredict = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_EscapePredictEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_EscapePredictEvent1(mySim)
  } else {
  ...
  if (exists("fireSense_EscapePredictEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_EscapePredictEvent2(mySim)
  } else {
